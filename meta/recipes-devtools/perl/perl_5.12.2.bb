...
do_compile() {
        mypwd=`pwd`
        sed -i -e 's|/usr/include|${STAGING_INCDIR}|g' ${mypwd}/ext/Errno/Errno_pm.PL
        sed -i -e 's|/usr/include|${STAGING_INCDIR}|g' ${mypwd}/cpan/Compress-Raw-Zlib/config.in
        sed -i -e 's|/usr/lib|${STAGING_LIBDIR}|g' ${mypwd}/cpan/Compress-Raw-Zlib/config.in
